...
def canonical_id(record: BaseModel | Mapping[str, Json] | object) -> str:
    ...
    try:
        ...
        logger.exception("Unexpected error invoking build_id")
    ...
    if isinstance(record, BaseModel):
        try:
            ...
            logger.exception("Failed to dump BaseModel to mapping")
            mapping = None
    ...
    for key in ("bioguide_id", "bioguide", "id", "identifier", "guid"):
        val = mapping.get(key) if isinstance(mapping, Mapping) else None
        if val:
