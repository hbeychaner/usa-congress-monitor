...
from cdm.data_collection.utils import extract_offset, resolve_pagination_wait
...
def enrich_records(
    ...
    *,
    ...
    for item in items_list:
        ...
        detail = retry_call(
            lambda current_item=item: detail_fetcher(current_item),
            retries=retries,
            backoff=backoff,
        )
...
def collect_with_details(
    *,
    ...
    page_size: int = 250,
    wait: float | None = None,
    list_checkpoint: Path | None = None,
    list_results: Path | None = None,
    detail_checkpoint: Path | None = None,
    detail_results: Path | None = None,
    retries: int = 3,
    backoff: float = 0.5,
) -> list[Mapping[str, Json]]:
