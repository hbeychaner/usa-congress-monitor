...
def test_ingest_crsreport(tmp_path, monkeypatch):
    repo = Path(__file__).resolve().parents[2]
    fixtures = repo / "tests" / "fixtures" / "crsreport"
    ...
    # spot-check: each produced item has an id and title or summary
    for a in actual[:5]:
        assert "id" in a
        assert "title" in a or "summary" in a or "description" in a
